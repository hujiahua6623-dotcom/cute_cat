...
**画布归一化坐标（`pets[].position` 与 `users[].pointer`）**：`x`、`y` 均为相对 Phaser 花园画布宽高的 0–1 值（左/上为 0）。`users[].pointer` 表示光标，可使用全画布。`pets[].position` 为权威存档；新领养宠物时服务端将 `y` 写入草地带（约 **0.62–0.80**），重叠散开时 `y` 钳制在约 **0.58–0.88**。Web 客户端绘制宠物精灵时，为兼容历史存档（曾使用更宽的 `y` 区间），会将 `position.y` 从约 **[0.2, 0.8] 单调映射**到展示用草地带 **[0.62, 0.80]**，**不改变**线上 JSON 字段含义与指针坐标语义。
